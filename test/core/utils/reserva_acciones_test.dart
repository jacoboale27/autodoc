...
void main() {
  group('calcularAccionesReserva (invariante: quien propone no resuelve)', () {
    ...
    test(
      'en estado pendiente, el proponente (propietario) NO puede aceptar ni cotizar ni rechazar ni reprogramar',
      () {
    ...
    test(
      'en estado pendiente, la contraparte (mecánico) SI puede cotizar y aceptar, rechazar y reprogramar',
      () {
        final acciones = calcularAccionesReserva(
          estado: 'pendiente',
          idProponente: userIdPropietario,
    ...
    test(
      'en estado pendiente tras reprogramación del mecánico, el mecánico NO puede resolver',
      () {
        final acciones = calcularAccionesReserva(
          estado: 'pendiente',
    ...
    test(
      'en estado pendiente tras reprogramación del mecánico, el propietario SI puede aceptar o reprogramar',
      () {
        final acciones = calcularAccionesReserva(
          estado: 'pendiente',
          idProponente: userIdMecanico,
    ...
    test(
      'en estado confirmada, ambas partes solo pueden cancelar con rastro',
      () {
    ...
    test(
      'en estado terminal (rechazada, cancelada, completada), no hay acciones activas',
      () {
        for (final estado in [
          'rechazada',
          'cancelada',
          'cancelada_por_propietario',
          'cancelada_por_taller',
          'completada',
        ]) {
          final acciones = calcularAccionesReserva(
            estado: estado,
    ...
    test('estadoCancelacionSegunRol devuelve sufijo según rol', () {
      expect(
        estadoCancelacionSegunRol(isMecanico: true),
        'cancelada_por_taller',
      );
      expect(
        estadoCancelacionSegunRol(isMecanico: false),
        'cancelada_por_propietario',
      );
    });
  });
