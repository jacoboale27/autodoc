...
/// Monta `AuthScreen` en modo alta, completa el formulario con datos validos
/// y lo envia con un `AuthService` mockeado que siempre tiene exito, para
/// dejar en pantalla el mismo dialogo de verificacion de correo que dispara
/// `_handleEmailRegister` en produccion (`auth_screen.dart:551`).
///
...
/// real: llenar el formulario y tocar "Crear cuenta". Solo soporta
/// `isRegistration: true` — el camino de login pasa por
/// `AuthProvider.needsEmailVerification`, que lee `FirebaseAuth.instance`
/// directamente (sin `Firebase.initializeApp()` en este test), asi que no es
/// alcanzable sin mockear Firebase Auth a ese nivel; no se necesita para el
/// hallazgo de esta tarea.
Future<void> pumpVerifyDialog(
  ...
  if (!isRegistration) {
    throw UnimplementedError(
      'pumpVerifyDialog solo soporta isRegistration: true; ver comentario.',
    );
  }
  ...
  await tester.pumpWidget(
    MultiProvider(
      ...
      child: MaterialApp(
        ...
        home: const AuthScreen(isLogin: false),
      ),
    ),
  );
