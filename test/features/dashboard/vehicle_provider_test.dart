...
void main() {
  ...
  group('VehicleProvider Tests', () {
    ...
    test('addVehicle success', () async {
      final vehicle = VehicleModel(idVehiculo: '1', idPropietario: 'owner', placa: 'ABC', marca: 'Toyota', modelo: 'Corolla', anio: 2020, color: 'Rojo');
      when(mockVehicleImageService.getVehicleImage(
      ...
      when(mockVehicleService.getVehiclesByOwner('owner')).thenAnswer((_) async => [vehicle]);
      when(mockVehicleService.getSharedVehicles('owner')).thenAnswer((_) async => []);
    ...
    test('deleteVehicle success', () async {
      ...
      when(mockVehicleService.getVehiclesByOwner('owner')).thenAnswer((_) async => []);
